...
public class R2RMLUtil {
	...
	public static String createJointQuery(TriplesMap child, TriplesMap parent, List<Join> joins) {
		...
		/*String query = "SELECT * FROM `" + cquery + "` 'AS' c ";
		query += ", ";
		query += "(SELECT * FROM `" + pquery + "`) 'AS' p WHERE ";*/
		
		
		String query = "SELECT * FROM (" + cquery + ") AS c, "; 
        query += "(" + pquery + ") AS p WHERE ";
		query += joins.stream().map(join -> "c." + join.getChild() + "=p." + join.getParent()).collect(Collectors.joining(" AND "));
		return query;
